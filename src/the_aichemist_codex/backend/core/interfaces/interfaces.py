...
class MetadataManager(ABC):
    ...
    async def search_metadata(
        self, query: str, file_extensions: list[str] | None = None, limit: int = 10
...
class SearchEngine(ABC):
    ...
    async def multi_search(
        self, query: str, search_types: list[str], options: dict[str, Any] | None = None
    ...
    @abstractmethod
    async def index_file(self, file_path: str, file_type: str | None = None) -> None:
        """
...
class FileTree(ABC):
    ...
    @abstractmethod
    async def get_tree(self, root_dir: str, max_depth: int | None = None) -> dict:
        """
    ...
    async def find_files(
        self, root_dir: str, pattern: str, max_depth: int | None = None
    ) -> list[str]:
        """
...
class RelationshipManager(ABC):
    ...
    async def detect_relationships(
        self, file_path: Path, strategies: list[str] | None = None
    ) -> list[dict[str, Any]]:
        """
...
class FileManager(ABC):
    ...
    async def create_version(
        self, file_path: Path, version_tag: str | None = None
    ) -> str:
        """
    ...
    async def restore_version(
        self, file_path: Path, version_id: str, restore_path: Path | None = None
    ) -> Path:
        """
    ...
    async def find_duplicates(
        self, directory: Path, recursive: bool = True, compare_method: str = "hash"
    ) -> dict[str, list[Path]]:
        """
