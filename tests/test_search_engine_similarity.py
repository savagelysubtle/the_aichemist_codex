...
@pytest.fixture
def mock_similarity_provider():
    """Create a mock similarity provider for testing."""
    mock_provider = MagicMock()

    # Mock the search method
    async def mock_search(query, file_paths=None, threshold=0.7, max_results=10):
        if "Python" in query:
    ...
    async def mock_find_similar_files(file_path, threshold=0.7, max_results=10):
    ...
    async def mock_find_file_groups(file_paths=None, threshold=0.7, min_group_size=2):
...
@pytest.fixture
def search_engine(mock_similarity_provider, mock_config):
    ...
    with patch("backend.search.search_engine.get_settings", return_value=mock_config):
        engine = SearchEngine(base_dir=Path("/test"))
...
class TestSearchEngineSimilarity:
    ...
    @pytest.mark.asyncio
    async def test_find_similar_files_async(self, search_engine, temp_files):
    ...
    @pytest.mark.asyncio
    async def test_find_similar_files_with_nonexistent_file(self, search_engine):
    ...
    @pytest.mark.asyncio
    async def test_find_similar_files_with_similarity_disabled(self, search_engine):
    ...
    @pytest.mark.asyncio
    async def test_find_file_groups_async(self, search_engine, temp_files):
    ...
    async def test_find_file_groups_with_specific_paths(
        ...
        # Verify that search_provider.find_file_groups was called with the right parameters
    ...
    @pytest.mark.asyncio
    async def test_find_file_groups_with_similarity_disabled(self, search_engine):
