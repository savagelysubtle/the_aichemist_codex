...
def test_store_initialization(temp_db_path):
    ...
    store = RelationshipStore(temp_db_path)
    ...
    with sqlite3.connect(str(temp_db_path)) as conn:
        ...
        # Check indexes
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_source_path'"
        )
        assert cursor.fetchone() is not None

        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_target_path'"
        )
        assert cursor.fetchone() is not None
...
def test_add_relationship(relationship_store, sample_relationships):
    ...
    assert retrieved.source_path == rel.source_path
    assert retrieved.target_path == rel.target_path
    assert retrieved.rel_type == rel.rel_type
    assert retrieved.strength == rel.strength
    assert retrieved.metadata == rel.metadata


def test_add_relationships(relationship_store, sample_relationships):
...
def test_get_relationship_not_found(relationship_store):
    """Test getting a relationship that doesn't exist."""
    retrieved = relationship_store.get_relationship("non-existent-id")
    assert retrieved is None


def test_get_relationships_for_file(relationship_store, sample_relationships):
    ...
    assert rels_filtered[0].rel_type == RelationshipType.IMPORTS


def test_get_related_files(relationship_store, sample_relationships):
    ...
    assert sample_relationships[0].target_path in related_paths
    assert sample_relationships[1].target_path in related_paths

    # Filter by relationship type
    related_files_filtered = relationship_store.get_related_files(
        source1, rel_types=[RelationshipType.IMPORTS]
    )
    assert len(related_files_filtered) == 1
    assert related_files_filtered[0][1] == RelationshipType.IMPORTS

    # Filter by minimum strength
    related_files_strong = relationship_store.get_related_files(
        source1, min_strength=0.8
    )
    assert len(related_files_strong) == 1
    assert related_files_strong[0][2] >= 0.8


def test_delete_relationship(relationship_store, sample_relationships):
    ...
    assert relationship_store.get_relationship(rel_id) is None
...
def test_delete_relationships_for_file(relationship_store, sample_relationships):
...
def test_clear_all_relationships(relationship_store, sample_relationships):
    ...
    assert count == len(sample_relationships)

    # Check that all relationships were deleted
    for rel in sample_relationships:
        assert relationship_store.get_relationship(rel.id) is None


def test_get_all_relationships(relationship_store, sample_relationships):
    ...
    assert len(all_rels) == len(sample_relationships)
    ...
    assert imports_rels[0].rel_type == RelationshipType.IMPORTS
