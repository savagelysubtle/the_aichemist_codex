"""
Tag classifier for automatic tag assignment based on file content and metadata.

This module provides the TagClassifier class, which uses machine learning
to automatically classify files and assign appropriate tags based on
...
class TagClassifier:
    """
    Classifies files and suggests appropriate tags based on content and metadata.

    This class uses machine learning models to automatically classify files
    and suggest tags. It can be trained on existing tagged files to improve
    ...
    def __init__(self, model_dir: Optional[Path] = None):
        ...
        """
        self.model_dir = model_dir or Path.home() / ".aichemist" / "models"
    ...
    async def load_model(self) -> bool:
        ...
        try:
            ...
            with open(self.metadata_path, "r") as f:
                ...
                self.tag_names = self.training_metadata.get("tag_names", [])

            logger.info(f"Loaded tag classifier model with {len(self.tag_names)} tags")
            return True

        except Exception as e:
            logger.error(f"Error loading tag classifier model: {e}")
    ...
    def _extract_features(self, file_metadata: FileMetadata) -> str:
        ...
        if hasattr(file_metadata, "topics") and file_metadata.topics:
            for topic, score in file_metadata.topics:
    ...
    async def get_tag_features(
        ...
        try:
            ...
            logger.error(f"Error getting features for tag {tag_name}: {e}")
            return []
